...
<img src="./defaas-imgs/用例图.svg" style="zoom:150%;" />
...
- 第五章介绍了一些重要的实现细节，这些细节用于：
  - 保护 Open 和 Fair 的逻辑没有漏洞
  - 说明 FaaS 平台的可行性
- 第六章指出了一些本设计的不足
- 第七章介绍了相关工作
- 第八章是结论与展望
...
<img src="./defaas-imgs/整体设计图.svg" style="zoom:150%;" />
...
>
> 我们这里不得不简化，舍去弹性的优势，计费方法类似于 PaaS，是按使用时长来计费，面向的是**大任务离线计算**这类函数计算：供应商以 PaaS 的方式出售计算资源，而租户只以 FaaS 的方式来利用。
>
> 安全性限制了本平台的应用范围。
>
...
- 监测结果为 True 表示证人认定供应商违约了（Violated）
- 监测结果为 False 表示证人认定供应商没有违约（NoViolated）
...
在 WitnessPool 中，以如下规则判断供应商是否违约：

- 对应一个 N，WintessPool 中会规定一个相对多数 M（N/2 < M < N），比如说 N = 3， M = 2
- 如果认定供应商违约的人数 W >= M，那么WitnessPool 合约就认定供应商违约；反之则不违约。
...
所以，由于证人之间的博弈制衡，证人委员会里的证人都将**如实报告**供应商是否违约，这形成了一个去中心化的可信第三方，依靠这个第三方， FaaS 平台维持了可信的 SLA，给租户带来了 Fairness。
...
### 5.5 如何把一个函数包成一个 docker image（未敲定的细节）

要求：

- 能够注入一些钩子，用于自动鉴权，证人检测等
...
- 自研：
  - 实现思路：在 docker container 中使用额外的程序拦截 HTTP 请求，获得注入能力
- 和现有 FaaS 框架的 image 兼容（如 OpenFaaS）
  - 好处：供应商一端可以直接使用这些框架来运行函数服务
  - 坏处：依赖太深，受牵制
