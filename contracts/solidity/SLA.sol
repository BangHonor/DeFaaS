...
contract SLA4FaaS is Owned {
    
    enum States { Fresh, Init, Active, Violated, Completed }

    States public SLAState;
    ...
    struct WitnessAccount {
        bool selected;   // 是否是证人委员会成员
        bool violated;   // 是否报告了 SLA is violated 
        uint balance;    // 服务本 SLA 合约的应得费用
    }
    ...
    // SLA 合约状态改变事件
    event SLAStateModifiedEvent(address indexed _who, uint _time, States _newstate);
    // 证人报告违约事件
    event SLAViolationReportEvent(address indexed _witness, uint _time, uint _roundID);
    ...
    // // SLA 合约状态检查
    // modifier atSLAState(States _state) {
    //     require(
    //         SLAState = _state,
    //         "SLA4FaaS: function cannot be called at this state"
    //     );
    //     _;
    // }
    ...
    // modifier witnessInCommittee() {
    //     require(
    //         witnesses[msg.sender].selected == true,
    //         "SLA4FaaS: witness is not in committee"
    //     );
    //     _;
    // }
